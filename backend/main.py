...
@app.get("/Afisarea_Meselor")
def afiseaza_mese(data : str, start: str, end: str, db: Session = Depends(get_db)):
    try:
        dt_start  = datetime.strptime(f"{data} {start[:5]}", "%Y-%m-%d %H:%M")
        dt_end = datetime.strptime(f"{data} {end[:5]}", "%Y-%m-%d %H:%M")
    except ValueError:
      raise HTTPException(status_code = 400, detail = "Format data/ora invalid")
  
    toate_mesele = db.query(meseDB).all()

    rezultat = []
    for m in toate_mesele :
        conflict = db.query(rezervareDB).filter(rezervareDB.masa == m.numar, dt_start < rezervareDB.check_out, dt_end > rezervareDB.check_in).first()
        
        rezultat.append({
            "numar": m.numar,
            "capacitate": m.capacitate,
            "ocupat": True if conflict else False
        })
    return rezultat

@app.get("/Are_rezervare")
def are_rezervare(db: Session = Depends(get_db), token = Depends(oath2scheme)):
    payload = jwt.decode(token, SECRET_KEY, algorithms=["HS256"])
    username = payload.get("sub")
    exista = db.query(rezervareDB).filter(rezervareDB.nume_client == username).first()
...
def inregistrarea_utilizatorului(user_Data : UserCreate, db: Session = Depends(get_db)):
    if(db.query(userDB).filter(user_Data.username == userDB.username)).first():
        return {"Error": "Acest username exista deja"}
