...
const Tabel_utilizatori = () => {
    ...
    const incarca_Utilizatori = async () => {
        try{
            ...
            const response = await fetch(`${import.meta.env.VITE_API_URL}/Utilizatori`, {
                method: 'GET',
                headers: {
    ...
    const handleStergere = async () => {
        ...
        try{
            const token = localStorage.getItem('token');
            const response = await fetch(`${import.meta.env.VITE_API_URL}/Stergere_Utilizator/${id_sters}`, {
                method: 'DELETE',
