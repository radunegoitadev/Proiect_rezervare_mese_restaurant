...
const Tabel_rezervari = () => {
    ...
    const incarca_Rezervari = async () => {
        try{
            ...
            const response =await fetch('http://localhost:8000/Afiseaza_Rezervarile', {method: 'GET', headers: {'Authorization': `Bearer ${token}`, 'Content-type': 'application/json'}})
    ...
    const handleStergere = async () => {
        ...
        try{
            const token = localStorage.getItem('token');
            const response = await fetch(`http://127.0.0.1:8000/Stergere_Rezervare/${id}`, {
                method: 'DELETE',
